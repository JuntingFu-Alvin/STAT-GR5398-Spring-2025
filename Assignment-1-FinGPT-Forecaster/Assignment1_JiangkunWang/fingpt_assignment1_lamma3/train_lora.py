...
class GenerationEvalCallback(TrainerCallback):
    ...
    def __init__(self, eval_dataset,tokenizer,max_length, ignore_until_epoch=0):
    ...
    def on_evaluate(self, args, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        if state.is_local_process_zero:
            model = kwargs['model']
            tokenizer = kwargs.get('processing_class', self.tokenizer)
            ...
            for feature in tqdm(self.eval_dataset):
                ...
                # print("Prompt Token Length:", len(inputs["input_ids"][0]))
                ...
                # answer = re.sub(r'.*prediction.\[\s*', '', output, flags=re.DOTALL)
...
def main(args):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        torch_dtype=torch.bfloat16,
      	 # rope_scaling={"name": "linear", "factor": 8.0}
    )
    if args.local_rank == 0:
        print(model)
    
    tokenizer = AutoTokenizer.from_pretrained(model_name, add_bos_token=False,trust_remote_code=True,local_files_only=True)
    ...
    print("2")
    dataset_list = load_dataset("dow30-202305-202405", True)
    # load_dataset(args.dataset, args.from_remote)
    ...
    model.config.use_cache = False  # 重要
    ...
    trainer = Trainer(
        ...
        callbacks=[
            GenerationEvalCallback(
                ...
                ignore_until_epoch=round(0.5 * args.num_epochs)
            )
        ]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base_model", required=True, type=str, choices=['llama2',"llama3-instruct-local","deepseekR1"])
