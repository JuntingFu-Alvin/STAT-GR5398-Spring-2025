...
def get_returns(stock_symbol, start_date, end_date):
    ...
    # 动态检查是否存在 'Adj Close' 或 'Close' 列
    if ('Adj Close', stock_symbol) in stock_data.columns:
        weekly_data = stock_data[('Adj Close', stock_symbol)].resample('W').ffill()
    elif ('Close', stock_symbol) in stock_data.columns:
        weekly_data = stock_data[('Close', stock_symbol)].resample('W').ffill()
    else:
        raise ValueError(f"Neither 'Adj Close' nor 'Close' column found for {symbol}.")
